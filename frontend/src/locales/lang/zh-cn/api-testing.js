...
export default {
  ...
  automation: {
    ...
    moveDown: '下移',
    addParam: '添加参数',
    paramTip: '参数会覆盖接口定义中同名的参数，留空值也可用于覆盖（如清除默认值）。变量支持 {{var}} 与 ${func()} 语法。',
    addHeader: '添加请求头',
    editCase: '编辑用例',
    ...
    bodyRaw: '原始文本',
    bodyOther: '保持原样（{type}）',
    bodyJsonPlaceholder: '输入JSON，例如：{"name": "{{name}}"}',
    bodyRawPlaceholder: '输入原始请求体文本',
    bodyEmptyTip: '选择 JSON 或原始文本以覆盖该接口的请求体',
    paramsFromInterface: '已自动带入该接口的原有参数，可直接修改；保存后以本列表作为该用例的参数（覆盖接口默认值）',
    headersFromInterface: '已自动带入该接口原有的请求头，可直接修改；保存后以本列表作为该用例的请求头（覆盖接口默认值）',
    interfaceBodyPreview: '接口默认请求体（仅预览）：',
    extractRules: '响应变量提取规则',
    extractTip: '执行完该用例后，按以下规则从响应中提取变量；后续用例可通过 {{变量名}} 引用。变量优先级：环境默认值 < 接口定义 < 用例覆盖 < 提取变量。',
    extractNamePlaceholder: '变量名',
    extractSourceBody: '响应体',
    extractSourceHeader: '响应头',
    extractSourceStatus: '状态码',
    extractJsonPathPlaceholder: 'JSONPath，如 $.data.token',
    extractHeaderNamePlaceholder: '响应头名称，如 Content-Type',
    addExtractRule: '添加提取规则',
    variablesBefore: '执行前变量',
  ...
  aiServiceConfig: {
    ...
    apiKeyHint: '当前密钥：{preview}，留空则保持原密钥',
  ...
  messages: {
    ...
    warning: {
      ...
      extractNameRequired: '请填写变量名',
      extractJsonPathRequired: '请填写JSONPath表达式',
      extractHeaderNameRequired: '请填写响应头名称',
