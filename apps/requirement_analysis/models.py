    ...
    async def generate_test_cases(task: TestCaseGenerationTask) -> str:
        ...
        user_message = (
            ...
            f"   - 对每个功能点，必须设计：1个正常场景 + 2-3个异常/边界场景。\n"
             f"3. **颗粒度**：用例颗粒度按功能复杂度决定，避免过度细化——简单功能以主流程 1-2 条 + 关键异常/边界覆盖即可；"
             f"同一输入框的多个子校验等不必要的细分项合并进所属场景用例；仅确实独立且重要的验证点单独成条，不为凑数拆分。\n"
            f"4. **场景扩展库**：\n"
        ...
        try:
            ...
            user_message = (
                ...
                f"2. **逻辑严密性**：检查预期结果是否具体、可验证（例如'提示错误'是不够的，需说明具体错误码或文案）。\n"
                 f"3. **冗余与必要度检查**：指出重复或无效的用例；同时指出相邻用例是否可合并，"
                 f"以及是否存在价值过低/过度细分（简单功能被拆成多条）的用例，建议精简或合并。\n\n"
                f"【待评审用例】\n{test_cases}\n\n"
                f"【输出格式要求】\n"
    ...
    def build_multimodal_messages(
        ...
        content_blocks.append({
            ...
            "text": (
                ...
                "【截图信息提取】\n"
                "· 流程图 → 梳理完整用户路径（含分支、回退、异常出口）\n"
                "· 原型图/UI稿 → 记录每个页面的元素清单、交互状态、跳转关系\n"
                "· 规则说明 → 逐条列出所有业务约束、数值定义、权限条件\n"
                "· 数据展示 → 注意表格结构、排序、筛选、分页规则\n\n"

                "【用例覆盖要求】\n"
                "· 截图中的每个页面、每个功能模块都要有对应用例，不跳过任何内容\n"
                "· 每条业务规则 → 正向验证 + 反向验证，确认规则生效和违规行为\n"
                "· 每个交互元素 → 覆盖正常态、禁用态、空状态、异常反馈\n"
                "· 每个用户路径 → 走完主线 + 覆盖分支和中断场景\n\n"

                "【通用场景补充】\n"
                "以下维度不依赖截图内容，也请自行补充：\n"
                "· 网络异常：断网、弱网、超时、切换网络\n"
                "· 设备状态：锁屏、切后台、杀进程、低电量\n"
                "· 数据边界：空值、特殊字符、超长文本、并发操作\n"
                "· 版本兼容：iOS/App 版本限制、不同分辨率适配\n\n"

                "【原则】\n"
                "· 按功能复杂度决定用例颗粒度，避免过度细化；不必要的细分项合并进所属场景用例\n"
                "· 先确保需求覆盖完整，再用澄清回答细化具体规则\n"
